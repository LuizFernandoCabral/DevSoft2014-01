...
aux1 = str.scan(/\[(.*)\]/)
aux2 = str.scan(/\[.*\] (.*)/)
aux3 = str.scan(/(\d\d:\d\d)/)

puts "\n"
for elemento in 0..aux1.length-1
 puts "#{aux1[elemento]}: #{aux2[elemento]} (#{aux3[elemento]})"
end
puts
...
# Seu código aqui...
cont = 0
str.each_line { |x| cont += 1 }
puts "Existem #{cont} linhas."
puts

# 3) Quantas notícias de economia de falam sobre o dólar?

# Seu código aqui...
aux1 = str.scan(/economia.*(dólar)/)
puts "#{aux1.length} notícias falam sobre o dólar."
puts

# 4) Imprimir todos os horários das notícias que contém a palavra golfe.
aux1 =  str.scan(/(\d\d:\d\d).*golfe/)
print "Horario das noticias sobre golfe: "
for elemento in 0..aux1.length-1
 print "#{aux1[elemento]} "
end
puts "\n\n"
